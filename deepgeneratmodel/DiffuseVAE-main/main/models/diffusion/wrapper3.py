...
from models.diffusion.ddpm_form3 import DDPMv3
from util import space_timesteps


class DDPMWrapper3(pl.LightningModule):
    def __init__(
        self,
        online_network,
        target_network,
        ...
        self.vae_loss_weight = vae_loss_weight  # VAE 손실 가중치 저장
    ...
    def forward(
        ...
        if sigma is not None:
            sigma_embed = self.sigma_embedding(sigma)  # Sigma에 대한 임베딩 추가
            x = torch.cat([x, sigma_embed], dim=1)  # 입력에 sigma 추가
        ...
        spaced_nw = (
            ...
            if isinstance(self.online_network, DDPMv3)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.conditional:
            x = batch
            # VAE 관련 계산
            #vae_recons, vae_mu, vae_logvar = self.vae(x)
            #vae_loss = self.vae.loss_function(vae_recons, x, vae_mu, vae_logvar)["loss"]
            with torch.no_grad():
                mu, logvar, sigma = self.vae.encode(x * 0.5 + 0.5)
        ...
        # VAE 관련 계산
        #vae_recons, vae_mu, vae_logvar = self.vae(x)
        vae_loss = self.vae.loss_function(cond, x, mu, logvar)["loss"]
        ...
        eps_pred = self.online_network(
            x, eps, t, sigma=sigma, low_res=cond, z=z.squeeze() if self.z_cond else None
        )
        ...
        # 하이브리드 손실 계산
        total_loss = vae_loss * self.vae_loss_weight + ddpm_loss
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            ...
            list(self.online_network.decoder.parameters()) + list(self.vae.parameters()), 
            lr=self.lr
        )
