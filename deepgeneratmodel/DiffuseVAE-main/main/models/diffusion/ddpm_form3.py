...
class DDPMv3(nn.Module):
    ...
    def forward(self, x, eps, t, low_res=None, z=None, sigma = None):
        if sigma is not None:
            # sigma를 x의 채널 크기에 맞추기 위해 리매핑
            sigma = sigma.view(sigma.size(0), sigma.size(1), 1, 1)  # [batch_size, latent_dim, 1, 1]
            if sigma.size(1) != x.size(1):  # 채널 크기가 다를 경우
                sigma = torch.nn.Conv2d(
                    in_channels=sigma.size(1),
                    out_channels=x.size(1),
                    kernel_size=1,
                    stride=1,
                    bias=True,
                ).to(x.device)(sigma)  # sigma 크기를 x 채널에 맞춤

            # 브로드캐스팅이 가능하도록 x에 sigma 곱하기
            x = x * sigma
