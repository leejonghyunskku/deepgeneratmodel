...
class VAE(pl.LightningModule):
    ...
    def encode(self, x):
        ...
        # 표준편차(sigma) 계산
        sigma = torch.exp(0.5 * logvar)  # logvar -> sigma
    ...
    def save_latent_space(self, dataloader, save_path="latent_space.pt"):
        self.eval()  # Evaluation 모드로 전환
        latent_list = []
        with torch.no_grad():
            for batch in dataloader:
                mu, logvar = self.encode(batch)
                z = self.reparameterize(mu, logvar)
                latent_list.append(z.cpu())  # GPU에서 CPU로 이동하여 저장
        latent_tensor = torch.cat(latent_list, dim=0)
        torch.save(latent_tensor, save_path)  # PyTorch의 텐서 저장
        print(f"Latent space saved to {save_path}")
